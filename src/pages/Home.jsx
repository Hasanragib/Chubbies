...
export default function Home() {
  const { homeUsps, franchise } = useSite()
      ...
      <section className={`section section--yellow ${styles.ctaSection}`}>
        <div className={`container ${styles.ctaGrid}`}>
          <div>
            ...
            <h2 className={styles.ctaHeading}>Own a Chubbies cart in your city.</h2>
            <p className={styles.ctaText}>
              {franchise.investment} all-in investment, FOFO model, no royalty — see what's included.
            </p>
            <Link to="/franchise" className={`btn btn--dark ${styles.ctaBtn}`}>View franchise details</Link>
          </div>
        </div>
      </section>
    </>
